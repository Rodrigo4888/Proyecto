...
class Localizacion(models.Model):
  ...
  def get_full_name(self):
      return self.distrito + " - " + self.provincia + " - " + self.departamento
...
class Pedido(models.Model):
    ...
    def get_total(self):
        ...
        if type(self.tarifa) != float:
            self.tarifa = 0
        total = self.tarifa + subtotal
        return total

    def get_direccion_entrega(self):
        dir = self.ubicacion.get_full_name() + " -- " + self.direccion_entrega
        return dir
